...
const ApprovedRefund = () => {
  ...
  const renderField = (field) => {
    if (typeof field === "object" && field !== null) {
      return JSON.stringify(field); // Convert object to string for rendering
    }
    return field || "N/A"; // Handle null/undefined cases
  };
      ...
      {loading ? (
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {refunds.map((refund) => (
              <tr key={refund.applyId}>
                <td>{renderField(refund.applyId)}</td>
                <td>{renderField(refund.adGoogleAccount)}</td>
                <td>${renderField(refund.amount)}</td>
                <td>${renderField(refund.remainMoney)}</td>
                <td>
                  <span className={`${styles.state} ${styles[refund.applyState?.toLowerCase()]}`}>
                    {renderField(refund.applyState)}
                  </span>
                </td>
                <td>{renderField(refund.adType)}</td>
                <td>{renderField(refund.refundReason)}</td>
                <td>{formatDate(refund.createdAt)}</td>
              </tr>
            ))}
          </tbody>
