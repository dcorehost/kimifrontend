  ...
  const handleCharge = async () => {
    ...
    try {
      const requests = rows.map(row => ({
        adBingAccount: row.id.trim(), // ✅ Fixed: Changed to "adBingAccount"
        money: parseFloat(row.money.trim()),
        adType: 'Bing', // ✅ Fixed: Always "Bing"
      }));
      ...
      const responses = await Promise.all(
        requests.map((requestData) =>
          axios.post(
            'http://admediaagency.online/kimi/create-bing-adDeposit', // ✅ Correct API URL
            requestData,
            {
              headers: {
