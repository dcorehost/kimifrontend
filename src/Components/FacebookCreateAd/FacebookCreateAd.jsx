...
const FacebookCreateAd = () => {
  ...
  const [adNumber, setAdNumber] = useState(1); // New state to manage ad numbers
  ...
  const handleAdNumberChange = (e) => {
    const num = parseInt(e.target.value, 10);
    setAdNumber(num);
    setAdsData(Array(num).fill({ accountName: "", deposit: "" })); // Update the adsData array dynamically
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "http://13.127.161.242:8001/kimi/create-facebook-ads",
        requestData,
        {
          headers: {
      ...
      <div className={styles.labelWithBorder}>
        ...
        {paymentMode === "oneByOne" && (
          <>
            <div className={styles.field}>
              <label className={styles.label}>
                <span>*</span> Select Ad Number:
              </label>
              <select
                className={styles.select}
                value={adNumber}
                onChange={handleAdNumberChange}
              >
            ...
            {adsData.slice(0, adNumber).map((ad, index) => (
              <div key={index} className={styles.field}>
                <label className={styles.label}>
                  <span>*</span> Account Name {index + 1}:
                </label>
                <input
                  type="text"
...
//   const [pageNum, setPageNum] = useState(2);
//   const [urls, setUrls] = useState(["",""]);
//   // const [urlsdeposite, setUrlsdeposite] = useState([""]);
...
//     setPageNum(num);
//     setUrls(Array(num).fill(""));
//     // setUrlsdeposite(Array(num).fill(""));
...
// const handleBack = () => {
//   // Logic for going back, could be navigation or a custom action
